...
sl_status_t lcd_tx(const struct sl_memlcd_t *device, const void *data, unsigned int row_start, unsigned int row_count) {
  ...
  if (row_count == 1) { // Cập nhật chỉ 1 dòng
      // Data Update Mode 1 line - Cập nhật dữ liệu cho 1 dòng
      /* Thiết lập SCS (Chip Select) - bật tín hiệu để bắt đầu giao tiếp SPI */
      GPIO_PinOutSet(SL_MEMLCD_SPI_CS_PORT, SL_MEMLCD_SPI_CS_PIN);

      /* Đợi một khoảng thời gian để thiết bị chuẩn bị (được chỉ định trong device->setup_us) */
      sl_udelay_wait(device->setup_us);

      for (row = row_start; row < row_start + 1; row++) {
          cmd = CMD_UPDATE | (row << 8); // Lệnh cập nhật dòng với địa chỉ bắt đầu là row_start
          sli_memlcd_spi_tx(&spi_handle, &cmd, 2); // Gửi lệnh cập nhật dòng (2 byte)

          // Gửi 16 byte dữ liệu cho dòng, chỉ cần gửi 1 lần duy nhất
          sli_memlcd_spi_tx(&spi_handle, data_ptr, 16); // Gửi 16 byte dữ liệu cho dòng (độ rộng 128 pixel = 16 byte)

          /* Cho phép dữ liệu được gửi */
          sli_memlcd_spi_wait(&spi_handle);
      ...
      /* Xóa SCS sau khi cập nhật xong */
      GPIO_PinOutClear(SL_MEMLCD_SPI_CS_PORT, SL_MEMLCD_SPI_CS_PIN);

      // Tạm dừng trong một khoảng thời gian nhỏ để điều khiển tần suất cập nhật
      sl_udelay_wait(1000);  // Chờ 1ms trước khi kết thúc
  } else { // Cập nhật nhiều dòng
      /* Thiết lập SCS (Chip Select) - bật tín hiệu để bắt đầu giao tiếp SPI */
      GPIO_PinOutSet(SL_MEMLCD_SPI_CS_PORT, SL_MEMLCD_SPI_CS_PIN);
      ...
      // Data Update Mode Multiple line - Cập nhật dữ liệu cho nhiều dòng
      for (row = row_start; row < row_start + row_count; row++) {
          cmd = CMD_UPDATE | (row << 8); // Lệnh cập nhật dòng hiện tại
          sli_memlcd_spi_tx(&spi_handle, &cmd, 2); // Gửi lệnh cập nhật dòng (2 byte)

          // Gửi 16 byte dữ liệu cho dòng hiện tại (128 pixel = 16 byte)
          sli_memlcd_spi_tx(&spi_handle, data_ptr, 16);

          // Chuyển con trỏ dữ liệu đến dòng tiếp theo
          data_ptr += 16; // 16 byte cho mỗi dòng
      }
